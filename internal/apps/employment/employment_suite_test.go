...
var (
	DB                  *gorm.DB
	EmploymentAPI       employment.EmploymentAPIServer
	EmploymentAPIServer *employmentAPIServer
	err                 error
)
...
var _ = BeforeSuite(func() {
	...
	EmploymentAPI, err = NewEmploymentAPI(ctx, DB)
	Expect(err).ShouldNot(HaveOccurred())

	var ok bool
	EmploymentAPIServer, ok = EmploymentAPI.(*employmentAPIServer)
	Expect(ok).Should(BeTrue())

	// When creating a employment server with bad credentials
	_, err = NewEmploymentAPI(nil, DB)
	Expect(err).Should(HaveOccurred())

	_, err = NewEmploymentAPI(ctx, nil)
	Expect(err).Should(HaveOccurred())
})
