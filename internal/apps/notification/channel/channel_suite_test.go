...
var (
	DB         *gorm.DB
	ChannelAPI channel.ChannelAPIServer
	err        error
)
...
var _ = BeforeSuite(func() {
	...
	// Inject stub to the service
	ChannelAPI, err = NewChannelAPIServerServer(ctx, DB)
	Expect(err).ShouldNot(HaveOccurred())

	// Fails with missing stubs
	_, err = NewChannelAPIServerServer(nil, DB)
	Expect(err).Should(HaveOccurred())

	_, err = NewChannelAPIServerServer(ctx, nil)
	Expect(err).Should(HaveOccurred())
})
