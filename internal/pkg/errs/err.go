...
// MissingCredential returns a status error caused by missing credentials
func MissingCredential(credential string) error {
	return status.Errorf(codes.InvalidArgument, "missing credential: %v", credential)
}
...
func FailedToParseToken(err error) error {
	return status.Errorf(codes.Internal, "failed to parse jwt token: %v", err)
}
...
// WrapErrorWithCodeAndMsg wraps generic error to status error of provided code and msg
