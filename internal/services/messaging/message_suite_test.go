...
import (
	...
	"github.com/gidyon/umrs/internal/mocks"
...
var _ = BeforeSuite(func() {
	...
	opt := &Options{
		...
		EmailSender:      "umrs test team",
		JWTSigningKey:    "who can guess :)",
	...
	opt.EmailSender = "umrs test team"
	opt.JWTSigningKey = ""
	_, err = NewMessagingServer(ctx, opt)
	Expect(err).Should(HaveOccurred())

	opt.JWTSigningKey = "agiain, damn secured"
	opt.PushClient = nil
	_, err = NewMessagingServer(ctx, opt)
	Expect(err).Should(HaveOccurred())
